...
export const useSpidersStore = create<SpidersState>((set, get) => ({
  spiders: [],
  nextId: 1,
  db: null,
  ...
  addSpider: async (spider: Omit<SpiderDB, "id">) => {
    ...
    db.insert(spiders).values(newSpider).run();
  ...
  updateSpider: async (id: number, updatedSpider: Omit<SpiderDB, "id">) => {
    ...
    db.update(spiders).set(updatedSpider).where(eq(spiders.id, id)).run();
